...
public class ReplyController {
	...
	//######################################리뷰 작성 공간###############################################
	//js는 /resources/reply/replyFunction.js로 연결하였습니다.
	@PreAuthorize("isAuthenticated()") //현 사용자가 로그인 처리 했습니까?
	@PostMapping(value = "write/{productId}",
			consumes = "application/json",
			produces = {MediaType.TEXT_PLAIN_VALUE})
	public ResponseEntity<String> insertReview(@AuthenticationPrincipal Principal principal,
			@PathVariable("productId") String productId, @RequestBody ReplyVO reply) {
		//로그인한 사용자로 작성자 연동
		UsernamePasswordAuthenticationToken upat = (UsernamePasswordAuthenticationToken) principal;
		CustomUser cu = (CustomUser) upat.getPrincipal();
		Party writer = cu.getCurUser();
		...
		//productId를 ownerId로 연결
		reply.setOwnerId(productId);
		System.out.println(reply);
		int insertCount = replyService.insertReview(reply);
		if (insertCount == 1) {
			return new ResponseEntity<>(reply.getId(), HttpStatus.OK); } 
		else { 
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR); }
