...
public class HashTagService {
	...
	public void mngHashTagAndMapping(Party curUser, Map<String, Integer> mapSearchWord) {
		...
		if (setSearchWord.isEmpty()) {
			//검색했을때 단어가 나타나지 않았으면 처리할 것이 없군요
			return;
		}
	
		//기존에 검색해서 활용되었고 활용 횟수 1 증가 시킬 대상
		Set<HashtagVO> setPrevUsed = hashTagMapper.findPrevUsedHashTag(curUser, setSearchWord); //기존에 검색에서 활용된 단어들
		Set<HashtagVO> setExisting = hashTagMapper.findExisting(setSearchWord); //내가 검색한 단어들 중에서 해쉬태그 테이블에 등록된 단어
